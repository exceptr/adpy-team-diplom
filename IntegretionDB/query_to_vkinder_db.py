...
def select_favorite(vk_id):
    ...
    for link in favorite_records:
        ...
        res = '\n'.join(new_list)
    return res
# print(select_favorite(144344533))

# def select_query_city():
#     cursor = connection.cursor()
#     query_city = "Select * from city"
#     cursor.execute(query_city)
#     city_records = cursor.fetchall()
#     return city_records[-1][0]


# def insert_data_usercity():
#     try:
#         city_id = select_query_city()
#         vk_id = select_query_vkuser()
#         cursor = connection.cursor()
#         insert_query = f"Insert into cityuser(id_city, id_user) values ({city_id}, {vk_id});"
#         cursor.execute(insert_query, vk_id)
#         connection.commit()
#         count = cursor.rowcount
#         print('Запись добавлена', count)
...
# def delete_data(city_id):
#     try:
#         cursor = connection.cursor()
#         delete_query = f"Delete from vkuser where id = {city_id}"
